        ...
        @foreach($weeks as $week)
            @foreach($week as $day)
                ...
                <a class="calendar-day {{ $day->month !== $month->month ? 'is-outside' : '' }} {{ $day->isToday() ? 'is-today' : '' }} {{ $selectedDate === $day->toDateString() ? 'is-selected' : '' }}" href="{{ route('events.index', ['mes' => $day->format('Y-m'), 'dia' => $day->toDateString()]) }}" aria-label="Ver detalles del {{ $day->translatedFormat('d F') }}"><span class="calendar-date">{{ $day->day }}</span>
                    @foreach($dailyEvents->take(3) as $event)<div class="calendar-event {{ $event->estado === 'cancelado' ? 'opacity-50 text-decoration-line-through' : '' }}" title="{{ $event->titulo }}{{ $event->lugar ? ' · '.$event->lugar : '' }}{{ $event->estado === 'cancelado' ? ' · Cancelado' : '' }}"><strong>{{ $event->titulo }}</strong>@if($event->estado === 'cancelado')<small>Cancelado</small>@elseif(!$event->todo_el_dia)<small>{{ \Illuminate\Support\Carbon::parse($event->inicio_at)->format('H:i') }}</small>@endif</div>@endforeach
...
<div class="card mb-4"><div class="card-header"><i class="bi bi-calendar-day me-2"></i>Detalle del {{ \Illuminate\Support\Carbon::parse($selectedDate)->translatedFormat('d \d\e F \d\e Y') }}</div><div class="card-body">
    @forelse($selectedEvents as $event)
        ...
        @endphp
        <article class="event-detail border-bottom pb-4 mb-4 last-child-no-border"><div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-3"><h3 class="h5 text-white mb-0 {{ $event->estado === 'cancelado' ? 'text-decoration-line-through opacity-75' : '' }}">{{ $event->titulo }}</h3>@if($event->estado === 'cancelado')<span class="badge text-bg-danger">Cancelado</span>@endif</div><dl class="row details-list mb-0"><dt class="col-md-3">Horario</dt><dd class="col-md-9">{{ $event->todo_el_dia ? 'Todo el día / por confirmar' : \Illuminate\Support\Carbon::parse($event->inicio_at)->format('H:i').' - '.\Illuminate\Support\Carbon::parse($event->termino_at)->format('H:i') }}</dd><dt class="col-md-3">Lugar / espacio</dt><dd class="col-md-9">{{ $event->lugar ?: 'Sin lugar definido' }}</dd><dt class="col-md-3">Responsable</dt><dd class="col-md-9">{{ $responsible ?: 'Sin responsable informado' }}</dd><dt class="col-md-3">Requerimientos técnicos y logísticos</dt><dd class="col-md-9 event-requirements">{{ $requirements ?: ($details ?: 'Sin requerimientos informados') }}</dd>@if($event->estado === 'cancelado')<dt class="col-md-3">Motivo de cancelación</dt><dd class="col-md-9">{{ ['reprogramado' => 'Reprogramado para otra fecha', 'disponibilidad' => 'Falta de disponibilidad de sala o equipo', 'suspendido' => 'Actividad suspendida', 'registro_error' => 'Evento registrado por error', 'duplicado' => 'Evento duplicado', 'no_realizado' => 'Actividad no realizada', 'otro' => 'Otro motivo'][$event->motivo_cancelacion] ?? 'Sin motivo informado' }}@if($event->observacion_cancelacion)<div class="small text-body-secondary mt-1">{{ $event->observacion_cancelacion }}</div>@endif</dd>@endif</dl>@can('gestionar-eventos')@if($event->estado === 'programado')<div class="mt-3"><button class="btn btn-sm btn-outline-danger" type="button" data-bs-toggle="collapse" data-bs-target="#cancelar-evento-{{ $event->id }}"><i class="bi bi-x-circle me-1"></i>Cancelar evento</button><div class="collapse mt-3" id="cancelar-evento-{{ $event->id }}"><form method="POST" action="{{ route('events.cancel', $event->id) }}" class="card card-body bg-transparent border-danger-subtle">@csrf<div class="row g-2 align-items-end"><div class="col-md-5"><label class="form-label" for="motivo-{{ $event->id }}">Motivo</label><select class="form-select" id="motivo-{{ $event->id }}" name="motivo_cancelacion" required><option value="">Selecciona un motivo</option><option value="reprogramado">Reprogramado para otra fecha</option><option value="disponibilidad">Falta de disponibilidad de sala o equipo</option><option value="suspendido">Actividad suspendida</option><option value="registro_error">Evento registrado por error</option><option value="duplicado">Evento duplicado</option><option value="no_realizado">Actividad no realizada</option><option value="otro">Otro motivo</option></select></div><div class="col-md-5"><label class="form-label" for="observacion-{{ $event->id }}">Observación <span class="text-body-secondary">(obligatoria si eliges Otro)</span></label><input class="form-control" id="observacion-{{ $event->id }}" name="observacion_cancelacion" maxlength="1000"></div><div class="col-md-2"><button class="btn btn-danger w-100" type="submit">Confirmar</button></div></div></form></div></div>@endif
@endcan
</article>
    @empty
