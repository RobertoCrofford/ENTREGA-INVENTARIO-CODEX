...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        assetSearch.addEventListener('input', () => { const term = normalize(assetSearch.value.trim()); assetOptions.forEach((option) => option.hidden = term !== '' && !normalize(option.dataset.search).includes(term)); });
        assets.addEventListener('change', () => { const selected = assets.options[assets.selectedIndex]; if (selected?.dataset.location) origin.value = selected.dataset.location; });
