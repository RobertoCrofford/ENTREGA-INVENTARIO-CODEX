...
<form method="POST" class="card" action="{{ route('movements.store') }}">
    <div class="card-body">
        ...
        <div class="row g-3">
            ...
            <div class="col-md-4"><label class="form-label" for="tipo">Tipo</label><select id="tipo" name="tipo" class="form-select">@foreach(['entrada' => 'Entrada', 'salida' => 'Salida', 'devolucion' => 'Devolución', 'traslado' => 'Traslado'] as $value => $label)<option value="{{ $value }}" @selected(old('tipo') === $value)>{{ $label }}</option>@endforeach</select></div>
