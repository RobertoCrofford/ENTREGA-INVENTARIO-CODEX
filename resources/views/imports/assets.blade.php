...
<p class="text-body-secondary">Carga un CSV o Excel, valida sus filas antes de guardar y confirma solo los activos válidos.</p>
@if(session('warning'))<div class="alert alert-warning">{{ session('warning') }}</div>@endif
<div class="card mb-4"><div class="card-body"><a class="btn btn-outline-primary mb-3" href="{{ route('imports.assets.template') }}">Descargar plantilla CSV</a><form method="POST" action="{{ route('imports.assets.preview') }}" enctype="multipart/form-data">@csrf<div class="row g-3 align-items-end"><div class="col-md-8"><label class="form-label" for="archivo">Archivo CSV o Excel (.xlsx, máximo 5 MB)</label><input class="form-control" id="archivo" name="archivo" type="file" accept=".csv,.xlsx,text/csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" required>@error('archivo')<div class="text-danger small">{{ $message }}</div>@enderror</div><div class="col-md-4"><button class="btn btn-primary w-100">Validar archivo</button></div></div></form></div></div>
